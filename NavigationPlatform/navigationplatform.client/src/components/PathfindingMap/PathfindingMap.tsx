...
export const PathfindingMap: React.FC<PathfindingMapProps> = ({
  ...
  const currentPathPoints = path
    ?.map((id) => nodes.find((n) => n.id === id && n.floor === currentFloor))
    .filter(Boolean) as { x: number; y: number }[];

  const firstPoint = currentPathPoints[0];
  const secondPoint = currentPathPoints[1];

  const routeHeading =
    firstPoint && secondPoint
      ? Math.atan2(secondPoint.y - firstPoint.y, secondPoint.x - firstPoint.x) * (180 / Math.PI) + 90
      : 0;
    ...
    <div className="map-view-v4">
      ...
      <div className="map-view-v4__svg-wrapper">
        <svg ref={svgElement} viewBox={viewBox || "0 0 1000 1000"} className="MapView3d4" onClick={onSvgClick}>
          ...
          {firstPoint && (
            <g transform={`translate(${firstPoint.x}, ${firstPoint.y}) rotate(${routeHeading})`}>
              <circle r="22" fill="white" stroke="red" strokeWidth="4" />
              <polygon points="0,-20 12,12 0,6 -12,12" fill="red" />
            </g>
          )}
        </svg>
      </div>
